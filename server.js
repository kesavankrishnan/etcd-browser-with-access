...
var viewUser = process.env.VIEW_USER;
var viewPass = process.env.VIEW_PASS;
var logged_user = 'admin';
...
http.createServer(function serverFile(req, res) {
  ...
  if(req.url === '/'){
    req.url = '/index.html';
    if(logged_user === 'viewer')
        req.url = '/viewUser.html';
    else if(logged_user === 'admin')
        req.url = '/index.html';
...
//  if(!adminUser) return true;
  if(!adminUser || !viewUser) return true;
...
//  return (auth[1] === adminUser && auth[2] === adminPass)
if(auth[1] === adminUser && auth[2] === adminPass)
  {
      logged_user   =   "admin";
      return true;
  }
  else if(auth[1] === viewUser && auth[2] === viewPass)
  {
      logged_user   =   "viewer";
      return true;
  }
